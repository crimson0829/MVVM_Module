...
//右滑删除功能的开关,默认开
        ...
        val ta = context.theme
            .obtainStyledAttributes(attrs, R.styleable.SwipeMenuLayout, defStyleAttr, 0)
    ...
    override fun onMeasure(
        ...
        mHeight = 0
        var contentWidth =
            0 //2016 11 09 add,适配GridLayoutManager，将以第一个子Item(即ContentItem)的宽度为控件宽度
...
//平滑展开Menu
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean { //add by zhangxutong 2016 12 07 begin:
//禁止侧滑时，点击事件不受干扰。
        if (isSwipeEnable) {
            when (ev.action) {
...
// 判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。
    ...
    fun smoothExpand() { //Log.d(TAG, "smoothExpand() called" + this);
