...
/**
 * Mateiral Design 风格ProgressView
 */
...
// The last time the spinner was animated
    ...
    constructor(
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.LoadingProgressView))
        setAnimationEnabled()
    }
...
//----------------------------------
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val height: Int
        //Measure Width
        width = if (widthMode == MeasureSpec.EXACTLY) { //Must be this size
            widthSize
        ...
        //Measure Height
        height =
            if (heightMode == MeasureSpec.EXACTLY || widthMode == MeasureSpec.EXACTLY) { //Must be this size
                heightSize
     ...
     * Use this dimensions to setup the mBounds and paints.
     */
    ...
    /**
     * Set the mBounds of the component
     */
    private fun setupBounds(layout_width: Int, layout_height: Int) {
        ...
        circleBounds =
            if (!fillRadius) { // Width should equal to Height, find the min value to setup the circle
     ...
     */
    private fun parseAttributes(a: TypedArray) { // We transform the default values from DIP to pixels
        ...
        circleRadius = a.getDimension(
            R.styleable.LoadingProgressView_lpv_circleRadius,
            circleRadius.toFloat()
        ).toInt()
        fillRadius = a.getBoolean(R.styleable.LoadingProgressView_lpv_fillRadius, false)
        barWidth =
            a.getDimension(R.styleable.LoadingProgressView_lpv_barWidth, barWidth.toFloat()).toInt()
        rimWidth =
            a.getDimension(R.styleable.LoadingProgressView_lpv_rimWidth, rimWidth.toFloat()).toInt()
        val baseSpinSpeed =
            a.getFloat(R.styleable.LoadingProgressView_lpv_spinSpeed, spinSpeed / 360.0f)
        spinSpeed = baseSpinSpeed * 360
        barSpinCycleTime =
            a.getInt(R.styleable.LoadingProgressView_lpv_barSpinCycleTime, barSpinCycleTime.toInt())
                .toDouble()
        barColor = a.getColor(R.styleable.LoadingProgressView_lpv_barColor, barColor)
        rimColor = a.getColor(R.styleable.LoadingProgressView_lpv_rimColor, rimColor)
        linearProgress = a.getBoolean(R.styleable.LoadingProgressView_lpv_linearProgress, false)
        if (a.getBoolean(R.styleable.LoadingProgressView_lpv_progressIndeterminate, false)) {
            spin()
        }
        // Recycle
        a.recycle()
...
//----------------------------------
    override fun onDraw(canvas: Canvas) {
        ...
        if (isSpinning) { //Draw the spinning bar
            mustInvalidate = true
...
// do something, like changing the color
            ...
            if (mProgress != mTargetProgress) { //We smoothly increase the progress bar
                mustInvalidate = true
                val deltaTime =
    ...
    private fun updateBarLength(deltaTimeInMilliSeconds: Long) {
        if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
            ...
            if (timeStartGrowing > barSpinCycleTime) { // We completed a size change cycle
...
// animation starts smooth from here
     ...
     */
    var progress: Float
        get() = if (isSpinning) (-1).toFloat() else mProgress / 360.0f
        set(progress) {
            var progress = progress
            if (isSpinning) {
                mProgress = 0.0f
