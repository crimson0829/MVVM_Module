...
class AdapterDividerLine(private val mContext: Context) : ItemDecoration() {
    //divider对应的drawable
    ...
    constructor(context: Context, dividerSize: Int, mode: LineDrawMode?) : this(
        context,
        mode
    ) {
    ...
    private fun drawVertical(
        ...
        for (i in 0 until childCount) {
            ...
            val right = if (dividerSize == 0) left + dip2px(
                mContext,
    ...
    private fun drawHorizontal(
        ...
        for (i in 0 until childCount) { //分别为每个item绘制分隔线,首先要计算出item的边缘在哪里,给分隔线定位,定界
            ...
            val bottom = if (dividerSize == 0) top + dip2px(
                mContext,
                DEFAULT_DIVIDER_SIZE.toFloat()
