...
object ImageUtils {
    ...
    fun getBitmap(file: File?): Bitmap? {
        return if (file == null) null else BitmapFactory.decodeFile(file.getAbsolutePath())
    }
    ...
    fun getBitmap(data: ByteArray, offset: Int): Bitmap? {
        return if (data.size == 0) null else BitmapFactory.decodeByteArray(
            data,
            offset,
            data.size
        )
    ...
    fun addCornerBorder(
        src: Bitmap,
        @androidx.annotation.IntRange(from = 1) borderSize: Int,
        @ColorInt color: Int,
        @FloatRange(from = 0.00) cornerRadius: Float
    ): Bitmap? {
        return addBorder(
            src,
            borderSize,
    ...
    fun renderScriptBlur(
        src: Bitmap,
        @FloatRange(from = 0.00, to = 25.00, fromInclusive = false) radius: Float
    ): Bitmap {
...
///////////////////////////////////////////////////////////////////////////
    ...
    fun compressByQuality(
        ...
        if (isEmptyBitmap(src) || maxByteSize <= 0) return ByteArray(
            0
        )
        val baos = ByteArrayOutputStream()
